...
* **EnableWriteUnprotector**
  ...
  * Another issue may be that macOS is conflicting with the write protection from CR0 register, to resolve this we have 2 options:
    ...
    * Note: Some laptops(ex. Dell Inspiron 5370) even with MATs support will halt on boot up, in these cases you'll have two options:
      ...
      * Enable `DevirtualiseMmio` and follow [MmioWhitelist guide](https://k-pers.github.io/OpenCore-Install-Guide/extras/kaslr-fix.html)
...
* **AppleXcpmCfgLock** and **AppleCpuPmCfgLock**
  ...
  * Alternatively you can properly disable CFG-Lock: [Fixing CFG Lock](https://k-pers.github.io/OpenCore-Post-Install/misc/msr-lock.html)
* **AppleXcpmExtraMsrs**
...
* **Missing EC patch**:
  ...
  * If you don't have one, grab it here: [Getting started with ACPI](https://k-pers.github.io/Getting-Started-With-ACPI/)
* **IRQ conflict**:
...
* **NVRAM Failing**:
  * Common issue HEDT and 300 series motherboards, you have a couple paths to go down:
    * 300 Series Consumer Intel: See [Getting started with ACPI](https://k-pers.github.io/Getting-Started-With-ACPI/) on making SSDT-PMC.aml
    * HEDT(ie. X99): See [Emulating NVRAM](https://k-pers.github.io/OpenCore-Post-Install/misc/nvram.html) on how to stop NVRAM write, note that for install you do not need to run the script. Just setup the config.plist

* **RTC Missing**:
  * Commonly found on Intel's 300+ series(ie. Z370, Z490), caused by the RTC clock being disabled by default. See [Getting started with ACPI](https://k-pers.github.io/Getting-Started-With-ACPI/) on creating an SSDT-AWAC.aml
  * X99 and X299 have broken RTC devices, so will need to be fixed with SSDT-RTC0-RANGE. See [Getting started with ACPI](https://k-pers.github.io/Getting-Started-With-ACPI/) on creating said file
...
* If you're hitting the 15 port limit, you need to make an [USB Map](https://k-pers.github.io/OpenCore-Post-Install/usb/)
...
* You have the following SSDTs:
  * SSDT-UNC(if not, see [Getting started with ACPI](https://k-pers.github.io/Getting-Started-With-ACPI/) on creating said file)
...
* **Note**: On older Intel CPUs(ie. Penryn and older), it may be due to IRQ conflicts or the HPET device being disabled. To resolve, you have 2 options:
  * [SSDTTime's FixHPET Option](https://k-pers.github.io/Getting-Started-With-ACPI/ssdt-methods/ssdt-easy.html)
...
Make sure you have SSDT-GPIO in EFI/OC/ACPI and in your config.plist under ACPI -> Add in your config.plist. If you are still having issues, reference the [Getting Started With ACPI GPIO page](https://k-pers.github.io/Getting-Started-With-ACPI/Laptops/trackpad.html).
...
This is due to [XhciPortLimit breaking with macOS 11.3 and newer](https://github.com/dortania/bugtracker/issues/162), to resolve this, you **must** disable XhciPortLimit under Kernel -> Quirks. Please ensure you've [mapped your USB ports correctly](https://k-pers.github.io/OpenCore-Post-Install/usb/) before doing so.
